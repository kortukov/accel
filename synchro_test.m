...
for i = 6:6% size(files,1)
    ...
    for j = 1:size(st_times_vid) 
        difference(j) = st_times_vid(j)*1000 - st_times_eeg(j);
    end
    
    eeg_plus_const = (st_times_eeg + difference(1));
    
    
   delta =0; index=0;
    for j = 1:length(eeg_plus_const)
        delta = timing(:)'-eeg_plus_const(j)/1000;
        index(j) = find( abs(delta)==min(abs(delta)) );
    end
    index = index'
    
    space = [st_times_eeg(1):st_times_eeg(end)]' + difference(1);
    
    space_timecourse = zeros(length(space), 1);
    space_timecourse(st_times_eeg - st_times_eeg(1) + 1) = 1;
    
    eeg_stim_timecourse = zeros(length(stim_timecourse),1);
    eeg_stim_timecourse(index) = 1;
    figure();
    subplot(2, 1, 1);
    plot(timing, stim_timecourse); hold on;
    plot(timing, eeg_stim_timecourse); hold off;
    subplot(2, 1, 2);
    plot(space, space_timecourse);
