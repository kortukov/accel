...
files = dir('*data.mat');
a = [2,4,8,12,16,18,22,27,29,31,34]; %numbers of files with labels

for i = a%length(files)
...
%     k = 1:length(number_of_labeled);
    tpr = number_of_labeled/max(number_of_labeled);
    fpr = number_of_unlabeled/max(number_of_unlabeled);
