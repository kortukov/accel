...
% ======== model creation ========
% leaf = [5 10 20 50 100 200]
% col = 'rbcmyg'
% for j = 1 : length(leaf)
%     bag = TreeBagger(200, train_X, train_Y, 'Method', 'classification', ...
%         'OOBPred', 'On', 'MinLeafSize', leaf(j));
%     plot(oobError(bag), col(j))
%     hold on
% end
% xlabel 'number of grown trees'
% ylabel 'MSE'
% legend({'5', '10', '20','50','100','200'}, 'Location', 'NorthEast')
% hold off

tree_bag = TreeBagger(100, train_X, train_Y, 'Method', 'classification', ...
        'OOBPred', 'On');

test_X = feature_extraction(full_test_matrix);
test_Y = label_extraction(full_test_matrix);
% test_L = error(bag, test_X, test_Y)
% test_L_mean = mean(test_L);
Y_hat = str2num(cell2mat(predict(tree_bag, test_X)));
accuracy = sum(Y_hat == test_Y)/length(Y_hat);  
